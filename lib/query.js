 ...
 * @property {number} located_rn - номер аозиционируемой строки
 * @property {boolean} chunk - признак, что итоговый запрос будет на чсть данных (limit,offset)
 * @property {number} chunk_start - номер начала части отдаваемых данных
 * @property {number} chunk_real_start - номер начала части отдаваемых данных с запасом на 1 строку назад, если отдается часть не снала данных
    ...
    setFilter() {
        ...
        if (this.rawcontrol.treeMode) {
            ...
            if (res_filter.length === 0) {
                ...
                const hasChildSQL = `, exists (select 1 from (${this.sql}) q where q.${ctrl.hidField} = flt.${ctrl.keyField}) as _hasChilds`;
                this.sql = `
                ...
                const hasChildSQL = `, exists (select 1 from paths q where q.${ctrl.hidField} = flt.${ctrl.keyField}) as _hasChilds`;
                this.sql = `
                with recursive main as (${this.sql}),
