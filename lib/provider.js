...
class NFPostgresProvider extends NFProvider {
    ...
    async query(connect, sql, params, options, control) {
        ...
        const defaultOpts = { rowMode: 'array', returnRN: true };
        const { rowMode, returnRN, signal } = { ...defaultOpts, ...options };
        ...
        try {
            ...
            if (returnRN) fieldsMetadata[fieldsCount] = { name: 'r_n_', dataType: 'numb' };
            const response = {
                metaData: fieldsMetadata,
                rowMode,
                located: _query.located_rn,
                chunk: _query.chunk,
                chunk_start: _query.chunk_start,
                chunk_end: _query.chunk_end
            };
            // eslint-disable-next-line prefer-rest-params
            ...
            if (returnRN) {
                const rowNumberStart = (_query.chunk_real_start) ? _query.chunk_real_start : 0;
                data.forEach((curr, index) => {
                    curr.r_n_ = rowNumberStart + index;
                });
            }
            response.data = data;
            return response;
    ...
    async func(connect, func, params) {
        /*
       Выполнение функции в бд с биндингом совпадающих реальных параметров функции в виде именнованого вызова
