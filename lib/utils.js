...
/**
 * Формирование человекочитаемого сообщения об ощибке, возникшей в базе данных
 * @param {Error} e
 * @return {Promise<string>}
 */
    ...
    try {
        ...
        // foreign_key_violation
            ...
            if (mode === 'iu') {
                ...
                const [, foreignTablename] = message.match(/"(.*?)"/);
 ...
 * @param {Query} query - запрос приошебший обработку класом Query
 * @param {{sql: string, params: *[], missedParams: string[]}} frmQuery - обработанный запрос
 * @param {Array<*>} initialArguments - оригинальные параматры вызова метода провайдера
